...
classifier\levi{keywords are the features, no?}. They trained the NFR classifier
...
However, for requirements reuse analyzing them is a vital task.\levi{don't get
this intro. What is ``requirements reuse'' ?. And how is this related to
functional requirements?} Automatically extract structured information of
...
participant of activities), \emph{action} (the main functional operation),
 ...
 of the Agentive)\levi{too much detail?}, \emph{locational} (the location or
 ...
 framework employed techniques of semantic role labeling\levi{explain} and
 ...
 Frame)\levi{explain} functional cases extraction.
 ...
 SRSs\levi{?}. EFRF consists of the 10 functional requirement types mentioned
 ...
 semantic information\levi{is the goal to extract semantic information or to
 classify functional requirements?} from the requirements of automaker systems.
...
straightforward e.g. store and retrieve transaction\levi{transaction?}. On the
other side, NFRs are complicated and challenging to implement, making the
design to meet NFRs or design test case for them\levi{unclear}.
...
(bagging)\levi{is bagging an algorithm?} for training the classifier.
\levi{Bring to preprocessing section: The text trimming used stopwords
elimination, lemmatization, stemming, and sentences split.} The sentences are
...
for NFRs classification from user reviews than Naïve Bayes and J48\levi{is this
conclusion because they tested with multiple algorithms or because it is better
than the SOA, in which case this should be mentioned in the table}. The authors
...
problem.\levi{this sentence doesn't state anything} \levi{Features were obtained
by applying standard text mining technique and additional attribute
embellishment \levi{what is attribute embellishment?}.
For text mining technique Inverse Document Frequency (IDF) transform, Term Frequency
(TF) transform, lowercase transformation, minimum term frequency, stemmer, and
number of words were used.} The second stage involved removing numbers, 2-
letter words and other symbolic characters.\levi{What is the second stage? Is
this the second problem?} It used reviews of top 40 paid and free apps from ten
different categories.\\

Kutranvoic \etal ~ \cite {Kurtanovic:2017} performed automated analysis on
software requirements\levi{which kind?} and performed classification on FRs,
NFRs and subcategories of NFRs using supervised learning achieved using
SVM.\levi{make sure we introduce the acronym before}. Some NFRs had negligble
requirements\levi{what does negligible mean?} in the dataset and due to this
they were ignored. The additional dataset of user comments\levi{what is this?}
from Amazon was integrated into the main dataset to avoid the data imbalance
problem in NFR. \levi{Move to preprocessing section: The predictor used
text-preprocessing techniques such as removal of punctuations, removal of stop words, and lemmatization for feature
extraction.} The experiments were performed on a dataset provided by the RE
datatrack, i.e. 12 classes and 625 requirements in total.\\

 Abad \etal ~ \cite{Abad:2017} targeted two problems: the first one is the
 classification of FR and NFR and second classification of NFR into categories.
\levi{Preprocessing of text performed by applying: POS tagging, Entity tagging
and temporal tagging. As a next step feature co-occurrence and regular
expression used to increase the weight of influential words in the
datset\levi{don't get it, too much
 detail}. J.48 DT used for the classifying the FR and NFR.} For achieving the
 second goal topic Modeling unsupervised algorithm LDA and BTM
 applied\levi{unclear}. For topic\levi{which topic?} generation different
 ...
 BTM\levi{what are all the acronyms?}. learnt decision tree to classify the FRs
...
\levi{Move to preprocessing: The preprocessing of text involved, Part of Speech
(POS) tagging, entity tagging, and temporal tagging.}\\

Garzoli \cite {Garzoli:2013} classified data\levi{which data?} into five types:
...
the combat management system of a Naval Combat System.\levi{what does the
dataset consist of?} The goal of the study was to come up with a general
architecture for large-scale and adaptive requirement analysis. For
classification the multi VSM\levi{is this ML?} model was used and for lexical
...
requirement.\levi{shouldn't this go to the verification/traceability section?}\\
...
framework was able to classify 43 tests accurately.\\
	...
	Avesani \etal\cite{PAvesani} presented the study that dealt with the scalability problems which arised in managing the prioritization of a large number of requirements
