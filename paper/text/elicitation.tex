...
process, manually annotated(labelled) tweets were used and \emph{Naive Bayes} algorithm used for classification.\levi{annotated how, which ML
algorithm was used?}. The authors claimed with help of experiment results that
...
Jiang \etal \cite{Jiang:2014} mined user reviews in mobile application from 
...
\levi{explain what kind of clustering}. The proposed system also helps developers to decide requirements related to software revenue by considering economic factors. \levi{explain} It used two datasets of online reviews: one from the
...
process onto an existing military tool \emph{skiweb} in order to make a decision what action needs to be taken on a military command.\levi{which decision?}. The used software allows users to post events and information. Other users can update and add comment and info. The goal of adding learning capability is to help to find the additional information relevant to the post.  It used supervised
\emph{Naive Bayes} algorithm to classify the text document to find related
requirements i.e. documents to the discussion topic \levi{related to what?}. Furthermore, proposed recommender
...
produce a slower dimensional model\levi{why slower?} because of a smaller number of
...
part?} For identifying topics, an unsupervised agglomerative clustering algorithm is
applied to unstructured data\levi{what's the difference?}. The proposed system analyzes online
datasets\levi{which datasets?} that is gathered from stakeholders in natural language. 
